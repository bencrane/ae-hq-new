...
Secrets live in Doppler project **`ae-hq-new`**. Nothing reads `.env` files in deployed environments — the API and app containers boot via `doppler run` using the `DOPPLER_TOKEN` each Railway service is given.

## Local dev

```sh
pnpm install
doppler setup   # project: ae-hq-new, config: dev
doppler run -- pnpm dev:api    # :8080
doppler run -- pnpm dev:app    # :5173
pnpm dev:marketing             # :5174
```

## Required Doppler secrets (`ae-hq-new`)
...
3. Variables → add `DOPPLER_TOKEN` (a Doppler **service token** scoped to `ae-hq-new` / the right config).
...
2. Either add `VITE_PLATFORM_APP_URL` directly in Vercel env vars, or install the Doppler↔Vercel integration on `ae-hq-new`.
